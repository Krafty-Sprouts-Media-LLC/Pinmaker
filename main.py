...
for directory in [config.UPLOAD_DIR, config.TEMPLATE_DIR, config.PREVIEW_DIR, 
                 config.FONT_DIR, config.LOG_DIR]:
...
app.add_middleware(TrustedHostMiddleware, allowed_hosts=["pinmaker.kraftysprouts.com", "localhost"])
...
async def analyze_image(file: UploadFile = File(...)):
    ...
    try:
        ...
        analysis_result = await asyncio.to_thread(image_analyzer.analyze_image, str(file_path))
...
async def upload_font(file: UploadFile = File(...)):
    ...
    try:
        ...
        result = await asyncio.to_thread(font_manager.register_font, file.filename, content)
