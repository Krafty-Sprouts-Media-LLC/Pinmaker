...
class FontManager:
    def __init__(self):
        ...
        self.web_safe_fonts = {
            ...
            'Helvetica': {'family': 'Helvetica', 'type': 'sans-serif', 'web_safe': True},
            'Times New Roman': {'family': 'Times New Roman', 'type': 'serif', 'web_safe': True},
            'Georgia': {'family': 'Georgia', 'type': 'serif', 'web_safe': True},
            'Courier New': {'family': 'Courier New', 'type': 'monospace', 'web_safe': True},
            'Verdana': {'family': 'Verdana', 'type': 'sans-serif', 'web_safe': True},
            'Trebuchet MS': {'family': 'Trebuchet MS', 'type': 'sans-serif', 'web_safe': True},
    ...
    async def _extract_font_info(self, font_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            if font_path.suffix.lower() in ['.ttf', '.otf']:
                ...
                full_name = self._get_font_name(name_table, 4) or f"{family_name} {style_name}"
                ...
                italic = 'italic' in style_name.lower() or 'oblique' in style_name.lower()
    ...
    def _determine_font_type(self, family_name: str, style_name: str) -> str:
        ...
        if any(keyword in family_lower for keyword in ['mono', 'courier', 'console', 'code']):
            return 'monospace'
        
        # Check for serif indicators
        if any(keyword in family_lower for keyword in ['times', 'georgia', 'serif', 'garamond']):
            return 'serif'
        
        # Check for script/decorative indicators
        if any(keyword in family_lower for keyword in ['script', 'brush', 'hand', 'cursive']):
            return 'cursive'
    ...
    async def cleanup_unused_fonts(self) -> int:
        ...
        try:
            ...
            registered_paths = {Path(info['path']) for info in self.font_registry.values()}
            
            # Remove unregistered files
            for font_file in font_files:
                if font_file not in registered_paths and font_file.name != 'font_registry.json':
                    try:
                        font_file.unlink()
                        removed_count += 1
                    except Exception as e:
