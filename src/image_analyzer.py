...
class ImageAnalyzer:
    ...
    async def _extract_colors(self, image_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            cluster_hex = ["#{:02x}{:02x}{:02x}".format(*color) for color in cluster_colors]
            ...
            for (bbox, text, confidence) in ocr_results:
                if confidence > 0.5:  # Filter low confidence detections
                    ...
                    font_info.append({
                        ...
                        "text_type": self._classify_text_type(text, estimated_font_size)
                    })
            ...
            for i, (bbox, text, confidence) in enumerate(ocr_results):
                if confidence > 0.3:  # Lower threshold for text extraction
                    ...
                    text_elements.append({
                        ...
                        "bbox": [x1, y1, x2, y2],
                        "confidence": confidence,
                        "suggested_placeholder": self._suggest_placeholder(text.strip())
                    })
    ...
    async def _analyze_layout(self, image: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "layout_type": self._classify_layout_type(layout_regions, width, height)
            }
        
        except Exception as e:
            return {"error": f"Layout analysis failed: {str(e)}"}
    
    def _analyze_grid_structure(self, regions: List[Dict], width: int, height: int) -> Dict[str, Any]:
        ...
        return {
            ...
            "alignment_points": {"x": x_coords[:5], "y": y_coords[:5]}  # Limit for response size
        }
    
    def _classify_layout_type(self, regions: List[Dict], width: int, height: int) -> str:
                    ...
                    for i, result in enumerate(results[0].boxes.data):
                        ...
                        if conf > 0.3:  # Confidence threshold
                            image_regions.append({
                                ...
                                "type": self._classify_image_type(image, [int(x1), int(y1), int(x2), int(y2)]),
    ...
    async def _detect_images_by_segmentation(self, image: np.ndarray) -> List[Dict[str, Any]]:
                ...
                if area > (width * height * 0.02):  # Filter small regions
                    ...
                    if 0.5 <= aspect_ratio <= 2.0:  # Reasonable aspect ratio for images
                        image_regions.append({
                            ...
                            "type": self._classify_image_type(image, [x, y, x + w, y + h]),
                            "placeholder_tag": f"{{IMAGE_{i + 1}}}"
                        })
        ...
        try:
            ...
            if bg_colors:
                ...
                bg_hex = "#{:02x}{:02x}{:02x}".format(
                    int(avg_bg_color[2]), int(avg_bg_color[1]), int(avg_bg_color[0])  # BGR to RGB
                )
            else:
                bg_hex = "#ffffff"
            ...
            bg_type = "solid" if bg_variance < 1000 else "gradient" if bg_variance < 5000 else "pattern"
