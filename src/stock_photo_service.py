...
class StockPhotoService:
    def __init__(self):
        ...
        self.image_categories = [
            ...
            'lifestyle', 'abstract', 'minimal', 'design', 'workspace'
        ]
    ...
    async def get_random_image(self, width: int = 800, height: int = 600, category: str = None) -> Optional[str]:
        ...
        try:
            ...
            for api in apis:
                try:
                    image_url = await self._get_image_from_api(api, width, height, category)
                    if image_url:
                        return image_url
    ...
    async def _get_image_from_api(self, api: str, width: int, height: int, category: str = None) -> Optional[str]:
    ...
    async def _get_unsplash_image(self, width: int, height: int, category: str = None) -> Optional[str]:
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                image_url = data.get('urls', {}).get('custom') or data.get('urls', {}).get('regular')
    ...
    async def _get_pexels_image(self, width: int, height: int, category: str = None) -> Optional[str]:
    ...
    async def _get_pixabay_image(self, width: int, height: int, category: str = None) -> Optional[str]:
    ...
    async def get_themed_image(self, theme: str, width: int = 800, height: int = 600) -> Optional[str]:
