...
class PreviewGenerator:
    ...
    async def create_preview(self, template_path: str, analysis: Dict[str, Any], template_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            preview_image = await self._render_preview(
                root, width, height, analysis
            )
    ...
    async def _render_preview(self, svg_root: ET.Element, width: int, height: int, analysis: Dict[str, Any]) -> Image.Image:
        ...
        try:
            ...
            for rect in svg_root.iter('rect'):
                ...
                if (rect_x == '0' and rect_y == '0' and 
                    rect_width == svg_root.attrib.get('width') and 
                    rect_height == svg_root.attrib.get('height')):
    ...
    async def _render_image_placeholders(self, draw: ImageDraw.Draw, svg_root: ET.Element, analysis: Dict[str, Any]):
            ...
            for i, region in enumerate(image_regions):
                ...
                if image_type == 'real_photo':
                    ...
                    if stock_image:
                        ...
                        stock_image = stock_image.resize((img_width, img_height), Image.Resampling.LANCZOS)
                        ...
                        draw.rectangle([x1, y1, x2, y2], fill='#4CAF50', outline='#45a049', width=2)
                        ...
                        draw.rectangle([x1, y1, x2, y2], fill='#2196F3', outline='#1976D2', width=2)
                else:
                    # Keep as placeholder icon (gray with dashed border)
                    draw.rectangle([x1, y1, x2, y2], fill='#e0e0e0', outline='#cccccc', width=2)
                    
                    # Draw dashed border effect (simplified)
                    dash_length = 5
                    for x in range(x1, x2, dash_length * 2):
                        draw.line([x, y1, min(x + dash_length, x2), y1], fill='#cccccc', width=2)
                        draw.line([x, y2, min(x + dash_length, x2), y2], fill='#cccccc', width=2)
                    
                    for y in range(y1, y2, dash_length * 2):
                        draw.line([x1, y, x1, min(y + dash_length, y2)], fill='#cccccc', width=2)
                        draw.line([x2, y, x2, min(y + dash_length, y2)], fill='#cccccc', width=2)
                    ...
                    draw.text((text_x, text_y), placeholder_tag, fill='#666666', font=font)
    ...
    async def _render_text_elements(self, draw: ImageDraw.Draw, svg_root: ET.Element):
        ...
        try:
            for text_elem in svg_root.iter('text'):
                ...
                sample_text = self.sample_content.get(placeholder_text, placeholder_text)
    ...
    async def _render_decorative_elements(self, draw: ImageDraw.Draw, svg_root: ET.Element):
        """Render decorative elements like borders, shapes"""
        try:
            for rect in svg_root.iter('rect'):
                # Skip background and image placeholder rectangles
                if (rect.attrib.get('fill') in ['#e0e0e0', '#4CAF50', '#2196F3'] or
                    rect.attrib.get('stroke-dasharray')):
                    continue
                ...
                if stroke_color != 'none' and stroke_color:
                    draw.rectangle([x, y, x + width, y + height], outline=stroke_color, width=stroke_width)
